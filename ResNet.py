...
class UnifiedResNet(nn.Module):
    def __init__(self, num_classes):
        super(UnifiedResNet, self).__init__()
...
def lesion_by_group(model, dataloader, device, class_names):
    ...
    group_size = num_filters // 32  # Divide into 32 groups

    original_weights = last_conv_layer.weight.data.clone()  # Backup original weights
...
model = UnifiedResNet(num_classes=len(class_names))
model.to(device)
print(model)
